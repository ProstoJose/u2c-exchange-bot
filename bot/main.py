...
async def cmd_start(message: Message, state: FSMContext) -> None:
    await state.clear()
    session_factory = message.bot["db_session_factory"]
...
async def show_rate(call: CallbackQuery, state: FSMContext) -> None:
    cfg = call.bot["config"]
    session_factory = call.bot["db_session_factory"]
    rate_service: RateService = call.bot["rate_service"]
    ...
    try:
        await call.bot.send_message(cfg.admin_id, admin_text, parse_mode=ParseMode.MARKDOWN)
    except Exception:
...
async def submit(call: CallbackQuery, state: FSMContext) -> None:
    cfg = call.bot["config"]
    session_factory = call.bot["db_session_factory"]
...
async def main() -> None:
    ...
    # small helper: allow repository.upsert_user to get a session
    bot["config"] = cfg
    bot["db_session_factory"] = session_factory
    bot["rate_service"] = dp["rate_service"]
